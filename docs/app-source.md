...
<ion-content>
  ...
  <ion-card>
   <div text-center class="title">Found Devices</div>
    <ion-card-content>
      <ion-card (click)="showConnectDeviceDialog(d)" *ngFor="let d of devices">
        <ion-row>
          <ion-col>
            {{d.name}}
          </ion-col>
          <ion-col>
            <ion-icon name="checkmark" *ngIf="d.id == device.id"></ion-icon>
          </ion-col>
        </ion-row>
        <ion-row>
          {{d.id}}
    ...
    <div text-center>Make sure that bluetooth is enabled.</div>
  </ion-card>

  <ion-card>

    <div text-center class="header">Data-Editor</div>

    <ion-card-content>
      <form (ngSubmit)="addLine()">
        <ion-item>
          <ion-input type="text" [(ngModel)]="newLine.text" name="text" placeholder="Linecontent"></ion-input>
        </ion-item>
        <ion-item>
          <ion-label>Line</ion-label>
          <ion-select [(ngModel)]="newLine.line" name="line" required okText="Ok" cancelText="Abbrechen" placeholder="Select Line">
        ...
        <button ion-button type="submit" block>Print line</button>
      </form>
    </ion-card-content>
  </ion-card>

  <ion-card>
    <div text-center class="header">Background-Color</div>
    <ion-card-content>
      <ion-row>
        <ion-col *ngFor="let color of colors" [style.background]="color" class="color-block" tappable (click)="selectBackground(color)"></ion-col>
      </ion-row>
    </ion-card-content>
    <div text-center></div>
  </ion-card>
  ...
  <ion-card>
    <div text-center class="header">Font-Color</div>
    <ion-card-content>
      <ion-row>
  ...
  <div>
    ...
    <button ion-button block (click)="clearCanvas()">Reset Image</button>
  </div>

  <button ion-button block (click)="onUpload()">Send Image</button>
...
```css
ion-content, .scroll-content, ion-card{
  padding: 0;
  margin: 0;
  color: #ffffff;
}
